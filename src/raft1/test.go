...
func (ts *Test) checkOneLeader() int {
	...
	//fmt.Println("Checking for a single leader")
	for iters := 0; iters < 10; iters++ {
		...
		if len(leaders) != 0 {
			//fmt.Println("Leader found in term ", lastTermWithLeader, " is ", leaders[lastTermWithLeader][0])
...
func (ts *Test) nCommitted(index int) (int, any) {
	...
	//fmt.Println("nCommitted: ", count, " cmd: ", cmd)
	return count, cmd
}
